...
export default function CourseSection() {
  return (
    <section className="py-12 md:py-20 bg-[#111111] border-t border-white/5" id="premium-courses-section">
      <div className="max-w-7xl mx-auto px-6">
         
        {/* Section Header */}
        <div className="text-center max-w-2xl mx-auto mb-12 space-y-2">
          <span className="text-xs text-[#C8A04D] font-sans font-semibold tracking-wider block">
            엄선된 맞춤형 코스
          </span>
          <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
            프리미엄 케어 프로그램
          </h2>
          <div className="w-8 h-[1px] bg-[#C8A04D]/40 mx-auto mt-3" />
        </div>
          ...
          {COURSES_DATA.map((course, idx) => {
            return (
              <motion.div
                ...
                className="flex flex-col justify-start bg-[#161616] border border-neutral-800 rounded-[20px] p-5 sm:p-6 hover:border-[#C8A04D]/60 transition-all duration-300 shadow-xl relative group/card overflow-hidden"
                id={`course-item-${course.id}`}
              >
                {/* Top Badge & SVG Icon with minimal top space */}
                <div className="flex items-center justify-between mb-4">
                  <span className="px-2.5 py-0.5 rounded-full bg-[#C8A04D]/10 text-[#C8A04D] text-[10px] font-sans font-bold tracking-wider border border-[#C8A04D]/20">
                    {course.badge}
                  </span>
                  <div className="text-[#C8A04D] bg-[#C8A04D]/5 p-2 rounded-xl border border-[#C8A04D]/20">
                    {course.id === 'dry-care' && (
                ...
                <h3 className="text-2xl sm:text-3xl font-sans font-extrabold text-white tracking-tight leading-none text-left mb-4">
                  {course.title}
                </h3>

                {/* Thin divider before pricing */}
                <div className="w-full h-[1px] bg-neutral-800/60 mb-3" />
                ...
                <div className="space-y-2">
                  {course.options.map((opt, oIdx) => (
                    <div 
                      key={oIdx} 
                      className="flex items-center justify-between py-2 border-b border-neutral-800/40 last:border-b-0"
                    >
                      <span className="text-xs sm:text-sm font-sans font-medium text-neutral-400">{opt.duration}</span>
                      <span className="text-xl sm:text-2xl font-sans font-black text-[#C8A04D] tracking-tight">
                        {opt.price}
                      </span>
                    </div>
                  ))}
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex flex-col items-center text-center space-y-4">
          <p className="text-xs text-neutral-400 font-sans leading-relaxed max-w-xl italic">
